  ...
  if [ "$os_choice" == "1" ]; then
    ...
    while true; do
      ...
      if [ "$lang_choice" == "1" ]; then
        # ENG : Add Docker's official GPG key for Ubuntu:
        sudo apt-get update && \
        ...
        # ENG : Add the repository to Apt sources for Ubuntu:
        echo \
...
#-------------------------------------------------------------------------------------------------------------
        ...
        # FR : Ajoute la clé GPG officielle de Docker pour Ubuntu :
        sudo apt-get update && \
        sudo apt-get install -y ca-certificates curl && \
        sudo install -m 0755 -d /etc/apt/keyrings && \
        sudo curl -fsSL https://download.docker.com/linux/ubuntu/gpg -o /etc/apt/keyrings/docker.asc && \
        ...
        # FR : Ajoute le dépôt aux sources d'Apt pour Ubuntu :
        echo \
          "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.asc] https://download.docker.com/linux/ubuntu \
          $(. /etc/os-release && echo "$UBUNTU_CODENAME") stable" | \
          sudo tee /etc/apt/sources.list.d/docker.list > /dev/null && \
...
#-------------------------------------------------------------------------------------------------------------
    ...
    while true; do
      ...
      if [ "$lang_choice" == "1" ]; then
        # ENG : Add the repository to Apt sources for Debian/Kali:
  	...
  	sudo tee /etc/apt/sources.list.d/docker.list && \
        ...
        # ENG : Add Docker's official GPG key for Debian/Kali:
        sudo apt-get update && \
...
#-------------------------------------------------------------------------------------------------------------
       ...
       # FR : Ajoute la clé GPG officielle de Docker pour Debian/Kali :
        sudo apt-get update && \
        sudo curl -fsSL https://download.docker.com/linux/debian/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg && \
        sudo chmod a+r /etc/apt/keyrings/docker.gpg
        ...
        # FR : Ajoute le dépôt aux sources d'Apt pour Debian/Kali :
        echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/debian bookworm stable" | \
  	sudo tee /etc/apt/sources.list.d/docker.list && \
        sudo apt update
