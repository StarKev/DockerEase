...
while true; do
  # FR : Choix OS
  # ENG : Choose OS
  echo -e "${BLUE}Choose your OS / Choisissez votre OS:${NC}"
  echo "1. Ubuntu"
  echo "2. Kali Linux"
...
#-------------------------------------------------------------------------------------------------------------
    ...
    while true; do
      ...
      if [ "$lang_choice" == "1" ]; then
        # ENG: Remove Docker repository:
        sudo rm /etc/apt/sources.list.d/docker.list
        if [ $? -eq 0 ]; then
          echo -e "${GREEN}Docker repository removed successfully${NC}"
        else
          echo -e "${RED}Error removing Docker repository${NC}"
          exit 1
        fi
        ...
        # ENG: Uninstall Docker:
        sudo apt-get purge -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose
        if [ $? -eq 0 ]; then
          echo -e "${GREEN}Docker uninstallation successful${NC}"
        else
          echo -e "${RED}Error during Docker uninstallation${NC}"
          exit 1
        fi
...
#-------------------------------------------------------------------------------------------------------------
        ...
        # FR : Supprimer le repository Docker :
        sudo rm /etc/apt/sources.list.d/docker.list
        if [ $? -eq 0 ]; then
          echo -e "${GREEN}Repository Docker supprimé avec succès${NC}"
        else
          echo -e "${RED}Erreur lors de la suppression du Repository Docker${NC}"
          exit 1
        fi

        # FR : Supprimer la clé GPG Docker :
        sudo rm /etc/apt/keyrings/docker.gpg
        if [ $? -eq 0 ]; then
          echo -e "${GREEN}Clé GPG de Docker supprimée avec succès${NC}"
        else
        ...
        # FR : Désinstaller Docker
        sudo apt-get purge -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose
        if [ $? -eq 0 ]; then
          echo -e "${GREEN}Désinstallation de Docker réussie${NC}"
        else
          echo -e "${RED}Erreur lors de la désinstallation de Docker${NC}"
          exit 1
        fi
    ...
    echo -e "${RED}Invalid OS choice. Please try again.${NC}"
  fi
done